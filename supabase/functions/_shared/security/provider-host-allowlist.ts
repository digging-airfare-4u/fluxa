/**
 * Provider Host Allowlist Validator (Edge Function / Deno runtime)
 * Validates that external provider URLs are on the approved host:port allowlist.
 * Mirrors src/lib/security/provider-host-allowlist.ts for the Deno runtime.
 * Requirements: 5.5, 5.7, 8.2, 8.6
 */
...
/**
 * Load the allowlist from Deno environment variable.
 * Uses 60s cache to balance freshness and performance.
 * Requirements: 8.2
 */
export function loadAllowlist(): string[] {
  ...
  const envValue = Deno.env.get('PROVIDER_HOST_ALLOWLIST') || '';
...
/**
 * Sanitize error messages to remove sensitive data (API keys, auth headers).
 * Requirements: 8.1
 */

...
describe('image url helpers', () => {
  it('builds COS-compressed discover cover urls for home cards', () => {
    ...
    expect(decodeURIComponent(parsed.search.slice(1))).toContain('imageMogr2/thumbnail/960x>/quality/78/format/webp/strip');
  });
  ...
  it('does not double-append image transforms to already optimized COS urls', () => {
    const url = 'https://fluxa-1390058464.cos.ap-tokyo.myqcloud.com/foo/bar/image.jpg?imageMogr2/thumbnail/960x>/quality/78/format/webp/strip';
    expect(getDiscoverCoverImageUrl(url, 'home')).toBe(url);
  });
});

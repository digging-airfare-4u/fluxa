...
describe('invite and referral api and ui flow contract', () => {
  ...
  it('captures pending referral code during registration only', () => {
    ...
    expect(authPage).toContain('pending_referral_code');
    expect(authPage).not.toContain('pending_invite_code');
    expect(authPage).toContain('signUp');

    expect(authDialog).toContain('inviteCode');
    expect(authDialog).toContain('pending_referral_code');
    expect(authDialog).not.toContain('pending_invite_code');
    expect(authDialog).toContain('signUp');
  });

  it('attempts only referral auto redeem in app layout and keeps failures non-blocking', () => {
    const appLayout = readFileSync(resolve(process.cwd(), 'src/app/app/layout.tsx'), 'utf8');

    expect(appLayout).toContain('pending_referral_code');
    expect(appLayout).toContain('/api/referral/redeem');
    expect(appLayout).toContain('clearPendingReferralCode');
    expect(appLayout).toContain('shouldClearPendingReferralCode');
    expect(appLayout).not.toContain('pending_invite_code');
    expect(appLayout).not.toContain('/api/invite/redeem');
    expect(appLayout).not.toContain('clearPendingInviteCode');
    expect(appLayout).not.toContain('shouldClearPendingInviteCode');
    expect(appLayout).toContain('console.error');
  });

  it('removes the old invite membership redemption section from profile page', () => {
    const profile = readFileSync(resolve(process.cwd(), 'src/app/app/profile/page.tsx'), 'utf8');

    expect(profile).not.toContain('Invite Code');
    expect(profile).not.toContain('/api/invite/redeem');
    expect(profile).not.toContain('ALREADY_REDEEMED');
    expect(profile).not.toContain('CODE_USED');
    expect(profile).not.toContain('CODE_EXPIRED');
    expect(profile).not.toContain('INVALID_CODE');
    expect(profile).not.toContain('membership_expires_at');
    expect(profile).toContain('My Referral Code');
  });
